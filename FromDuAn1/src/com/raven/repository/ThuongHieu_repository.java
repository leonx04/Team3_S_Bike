...
public class ThuongHieu_repository {
    ...
    public List<Model_Thuonghieu> getAllTH() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                Model_Thuonghieu TH = new Model_Thuonghieu(rs.getString(1),
                        rs.getString(2));
                list.add(TH);
            }
            return list;
    ...
    public Model_Thuonghieu getTenThuongHieu(String ten) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                Model_Thuonghieu mdTH = new Model_Thuonghieu(rs.getString(1), rs.getString(2));
                return mdTH;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public int getID(String tenThuongHieu) {
        sql = "SELECT ID FROM ThuongHieu WHERE TenThuongHieu = ?";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
