...
public class ChiTietSanPham_repository {
    ...
    public int deleteCTSP(String maCTSP) {
    ...
    public int updateCTSP(Model_ChiTietSanPham sp, String maCTSP) {
        sql = " UPDATE ChiTietSanPham SET HinhAnh = ?, SoLuong = ? , GiaTien = ?, TrangThai =? , IDThuongHieu = ? , IDMauSac = ? ,  IDGhiDong = ?,  IDHeThongTruyenDong = ? , IDKhungXe = ? , IDLopXe = ? , IDPhanhXe = ?, IDBanhXe = ?   WHERE MaCTSP = ? ";
        con = DBConnect.getConnection();
        try {
            ps = con.prepareStatement(sql);
            ps.setObject(12, maCTSP);
            ps.setObject(1, sp.getHinhAnhXe());
            ps.setObject(2, sp.getSoLuong());
            ps.setObject(3, sp.getGiaTien());
            ps.setObject(4, sp.getTrangThai());
            ps.setObject(5, sp.getThuongHieu().getId());
            ps.setObject(6, sp.getMauSac().getID());
            ps.setObject(7, sp.getGhiDong().getID());
            ps.setObject(8, sp.getLoaiLip().getIDHTTD());
            ps.setObject(9, sp.getTenKhungXe().getID());
            ps.setObject(9, sp.getTenLopXe().getID());
            ps.setObject(10, sp.getTenPhanhXe().getID());
            ps.setObject(11, sp.getTenBanhXe().getID());
            return ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
